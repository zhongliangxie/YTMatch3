...
func slowly_larger():
	size_tween.interpolate_property(this_sprite, "scale", Vector2(1, 1), Vector2(1.5, 1.5), 2, Tween.TRANS_SINE, Tween.EASE_OUT)
	size_tween.start()

func slowly_dimmer():
	color_tween.interpolate_property(this_sprite, "modulate", Color(1, 1, 1, 1), Color(1, 1, 1, 0.2), 2, Tween.TRANS_SINE, Tween.EASE_OUT)
	color_tween.start()
